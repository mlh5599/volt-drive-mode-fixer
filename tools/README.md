...
| `soc_log.py` | Phase C.5 | Does **not** transmit. SOC-only successor to `drive_log.py` (no mode walk, no shift routine). Start it parked, `--minutes N`, drive the pack down. Spawns a full `candump -l` capture (`~/candump-<ts>.log` -- the deliverable), drops `SOC-MARK` anchors every `--mark-every` s, and with `--buttons` reads two panel buttons on the Pi header (`--button-a-gpio` 5 / `--button-b-gpio` 6, to GND, `gpiozero`) for hand marks the instant the EV range or a bar ticks -- tagged `BTN-RANGE` / `BTN-BAR`. Hold **both** buttons `--stop-hold` s to end cleanly without SSH; Ctrl-C (parked) also works. `--lcd` mirrors the `t+<s>` clock / marks / SAY prompt. `--dry-run` prints the plan. Mine the capture afterward with `mine_capture.py --monotonic`. |
