...
| `button_helper.py` | Phase C.5 | Does **not** transmit. Long-lived panel-button daemon (`systemd/voltdmf-btn.service`), dispatch by gesture (rules live in a side-effect-free `_Gestures` state machine, unit-tested in `tests/test_button_helper.py`): an **SW1 tap** runs `voltdmf-ctl setpoint <hold/mountain>` (toggles the reconciler setpoint -- a logged no-op until `setpoint` lands in `voltdmf-ctl`); **SW2 held `--single-hold-secs`** (default 5) *alone* then released launches `voltdmf-chargelog.service` (the charge-current-setpoint garage session, checklist §2e); **SW1+SW2 held `--launch-hold-secs`** (default 5, above `soc_log.py`'s 3s stop hold) launches `voltdmf-soclog.service`. Either launch claims the LCD hand-off lock, tears down the gpiozero pin factory to actually free the GPIO lines (a bare `Button.close()` leaves lgpio's `gpiochip` handle open and the kernel lines claimed), `systemctl start`s the oneshot, watches it via `systemctl is-active` until the capture ends, then re-acquires -- and waits for both pads to be released first so the capture's hold-BOTH stop can't roll into a fresh launch. Skips the GPIO grab if either capture unit is already active at startup (safe restart mid-run). Needs `gpiozero` -> system interpreter, not the daemon venv. `--dry-run` prints the resolved plan. |
